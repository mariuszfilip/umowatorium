...
class Form
{
    ...
    {
        //$this->oFormElementTable = $oFormModel;
     ...
     * @throws \Exception
     */
    public function getInfoFormularz($id_formularz)
    {
        if(!is_int($id_formularz)) {
            throw new \Exception('brak id Formularza');
        }
        $this->id_form = intval($id_formularz);
        return array();
       // return $this->oFormElementTable->getPolaFormularza($id_formularz);
    }
     ...
     */
    public function isValid($aForm, $aDane){
        return $this->validate($aForm, $aDane);
    }
     ...
     * @throws Exception
     */
    protected function validate($aFormTemp, $aDane)
    {
        $aForm  = array();
        foreach($aFormTemp as $aValue){
            $aForm[$aValue['nazwa_pola']]=$aValue;
        }
        $czyPrawidlowyFormularz = true;
        $Errors = array();
        //$aForm = $this->getInfoFormularz($id_formularz, array('walidacja_serwer', 'nazwa_pola','text_blednej_walidacji'));

        $aWalidacja = array();
        $aTextyBlednejWalidacji = array();
        //zapis wszystkich funkcji itextów walidacji pól fromularza
        foreach($aForm as $pole) {
            $pomWalidacja=\Zend\Json\Json::decode($pole['walidacja_serwer']);
            $aFunkcjeWalidacji[$pole['nazwa_pola']]=$pomWalidacja;
        }

        $oTextyWalidacji=new \My\Dhtmlx\Models\TextyWalidacji();


        foreach($aDane as $pole => $wartosc) {
            $czyPoprawnePole=null;
            if(isset($aFunkcjeWalidacji[$pole])){
                foreach($aFunkcjeWalidacji[$pole] as $func =>$text){
                    /*
                     * jezeli wyrazenie reg
                     * */
                    if (preg_match('/^\/\^.*\$\/$/', trim($func))) {
                        $czyPoprawnePole=preg_match( $func, $wartosc);
                    }else	if(method_exists('\My\Dhtmlx\Weryfikacja\Funkcje',$func)){
                        /*
                         * jezeli funkcja
                         * */
                        $czyPoprawnePole = \My\Dhtmlx\Weryfikacja\Funkcje::$func($wartosc);
                    } else if(!method_exists('My\Dhtmlx\Weryfikacja\Funkcje',$func)){
                        throw new \Exception("Błąd walidacji pola: ".$pole." ".__CLASS__.' '.__FUNCTION__.' funkcja: '.$func);
                    }
                    if(!$czyPoprawnePole){
                        $czyPrawidlowyFormularz=false;

                        if(intval($text)==0){
                            throw new \Exception("brak tekstu walidacji: ".$pole);
                        }
                        $Errors[$pole]=$oTextyWalidacji->getTextyById($text);
                        break;
                    }
                }
            }
    ...
    public function getError($format='sjson')
    {
        /**
         * json to string
         * */
    ...
    public function setSelectOption($nazwaPola, $aDane, $aSearch)
    {
        ...
        foreach($aSearch as $key => $val) {
            ...
            if($val['nazwa_pola'] == $nazwaPola) {
                $DoPodmiany[$key]['column'] = $nazwaPola;
                $DoPodmiany[$key]['tabela_slownikowa'] = $aDane;
            }
        }
        return $DoPodmiany;
    }
     ...
     */
    public function getXMLForm($aData = array(), $aDataValues = array())
    {
        ...
        if(is_array($aData) && count($aData) > 0) {
            ...
            foreach($aData as $key => $val) {
                if($val['id_nadrzednego'] == 0){
                    ...
                    $itemNew = $this->dodajElementPodrzedny($itemNew,$val,$aDataPosortowane,$aDataValues);
                }

            }
        }
        //$Items->appendChild($itemFieldset);
        if($this->bPokazPrzyciskZapisz) {
