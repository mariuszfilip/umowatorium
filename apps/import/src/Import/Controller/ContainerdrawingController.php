...
class ContainerdrawingController extends AbstractActionController
{
    protected $id_container = 0;

    protected $oContainerDrawingTable;

    public function uploadAction(){
        $id_container = (int)$this->getEvent()->getRouteMatch()->getParam('id_container');
        $aReturn = array();
        $aDataInsert = array();
        $sDestination =  ROOT_DIR .'/upload/container/'.$id_container.'/';
        (int)$id_user = $this->getAuthService()->getStorage()->read()->id;
        $aDataInsert['id_container']=$id_container;
        ...
        if ($adapter->isValid()) {
            ...
            if ($adapter->isUploaded()) {
                if ($adapter->receive()) {
                    $file = $adapter->getFileInfo();
                    $aDataInsert['picture'] = $file['file']['name'];
    ...
    public function uploadedAction(){
            ...
            $id_container = (int)$this->getEvent()->getRouteMatch()->getParam('id_container');
            $aData = $this->getContainerDrawingTable()->getContainerDrawing($id_container);
            $out = array();
            if (is_array($aData) && count($aData) > 0) {
                foreach ($aData as $key => $val) {
                    $sFile = ROOT_DIR .'/upload/container/'.$id_container.'/'. $aData[$key]["picture"];
                    if(file_exists($sFile)){
                        $aData[$key]["path"] = myURL .'/upload/container/'.$id_container.'/'. $aData[$key]["picture"];
                        //ToDo poprawic
    ...
    public function getContainerDrawingTable(){
        if(!$this->oContainerDrawingTable){
            ...
            $this->oContainerDrawingTable = $sm->get('ContainerDrawingTable');
        }
